...
from models.vgg import VGG_A_Dropout_BatchNorm # you need to implement this network
...
def plot_loss_landscape(vgg_min, vgg_max, vgg_bn_min, vgg_bn_max):
    ...
    # landscape of VGG-A-Dropout
    plt.fill_between(range(len(vgg_min)), vgg_min, vgg_max, 
                     alpha=0.3, label='VGG-A-Dropout', color='green')
    
    # landscape of VGG-A-Dropout-BatchNorm
    plt.fill_between(range(len(vgg_bn_min)), vgg_bn_min, vgg_bn_max, 
                     alpha=0.3, label='VGG-A-Dropout-BatchNorm', color='red')
    
    plt.title('Loss Landscape Comparison: VGG-A-Dropout vs VGG-A-Dropout-BatchNorm')
    plt.xlabel('Steps')
    plt.ylabel('Loss Landscape')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.savefig(os.path.join(figures_path, 'loss_landscape_2.png'))
    plt.close()
    
    print("Loss landscape comparison plot saved to:", 
          os.path.join(figures_path, 'loss_landscape_2.png'))
    

def main():
    learning_rates = [1e-3, 2e-3, 1e-4, 5e-4]
    
    print("Training VGG-A-Dropout models...")
    vgg_losses, vgg_grads = train_multiple_models(VGG_A_Dropout, learning_rates)
    
    print("\nTraining VGG-A-Dropout-BatchNorm models...")
    vgg_bn_losses, vgg_bn_grads = train_multiple_models(VGG_A_Dropout_BatchNorm, learning_rates)
